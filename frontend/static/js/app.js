...
// Ensure Flowing MVP Footer module is loaded when app.js runs (loads as module to allow ES imports)
(function loadFlowingFooterModule() {
  try {
    const tryInject = () => {
      // If a static script tag (data-flowing-footer) exists, don't inject
      if (document.querySelector('script[data-flowing-footer]')) return;
      // If any existing script references the footer src, don't inject
      ...
      s.type = 'module';
      s.src = '/static/js/flowing-mvp-footer.js';
      s.setAttribute('data-flowing-footer', '1');
      s.async = true;
  ...
  } catch (e) { console.warn('Could not load Flowing MVP Footer module', e); }
})();
...
async function renderKanban() {
  ...
  try {
    ...
    if (response.ok) {
      // Read as text and parse safely to avoid uncaught SyntaxError when backend
      // returns HTML/error pages or malformed JSON. This mirrors the robust
      // parsing used elsewhere in the app (issues fetch).
      try {
        const responseText = await response.text();
        if (!responseText) {
          kanbanData = null;
          ...
          try {
            ...
            console.warn('⚠️ Kanban backend returned non-JSON or malformed JSON. Falling back to local grouping.');
            console.warn('📄 Preview (first 500 chars):', responseText.substring(0, 500));
            kanbanData = null;
          }
        }
      } catch (readErr) {
        console.warn('⚠️ Could not read kanban response body:', readErr);
        kanbanData = null;
      }
    } else {
      console.warn('⚠️ Kanban fetch returned non-OK status', response.status, response.statusText);
    }
  } catch (error) {
