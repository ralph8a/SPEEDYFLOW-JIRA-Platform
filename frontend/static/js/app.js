 ...
 */
// Prevent duplicate app bundle initialization if multiple copies are loaded
if (window.__SPEEDYFLOW_APP_LOADED) {
  console.warn('⚠️ Duplicate SpeedyFlow app bundle detected - skipping second load');
} else {
  window.__SPEEDYFLOW_APP_LOADED = true;
      ...
      if (targetDesk) {
        ...
        if (queueSelect && targetDesk.queues && targetDesk.queues.length > 0) {
          ...
          if (targetQueue) {
            ...
            const optionExists = Array.from(queueSelect.options).some(opt => String(opt.value) === String(targetQueue.id));
    ...
    try {
      ...
      if (filterSelect) {
        ...
        try {
          ...
          if (desk_id) {
            const deskExists = Array.from(filterSelect.options).some(opt => String(opt.value) === String(desk_id));
            if (deskExists) {
              ...
              if (queue_id) {
                setTimeout(() => {
                  const queueSelect = document.getElementById('queueSelectFilter');
                  if (queueSelect) {
                    const queueExists = Array.from(queueSelect.options).some(opt => String(opt.value) === String(queue_id));
                    if (queueExists) {
  ...
  function applyCardLayout() {
    ...
    cards.forEach(card => {
      ...
      // Card interactions: legacy right-sidebar removed; Flowing MVP / footer handles details
    });
  }
  ...
  // Export openIssueDetails to global scope for onclick handlers (guarded)
  if (typeof openIssueDetails === 'function') {
    window.openIssueDetails = openIssueDetails;
  } else {
    // Provide a safe noop stub so other modules can call it without throwing
    window.openIssueDetails = function (issueKey) {
      console.warn('openIssueDetails not available. Issue key:', issueKey);
    };
  }
  /**
  ...
  // Expose refetchUsers globally
  window.refetchUsers = refetchUsers;

} // end __SPEEDYFLOW_APP_LOADED guard
